...
class Node():
    ...
    def compute_proba(self, alpha : float, time_delta : int, market : Market, is_div : bool, dividende : float):
        ...
        if is_div:
            forward_price = self.price * exp(market.rate * time_delta) - dividende
        else :
            forward_price = self.price * exp(market.rate * time_delta)
        variance = pow(self.price,2) * exp(2 * market.rate * time_delta) * (exp(pow(market.volatility, 2) * time_delta) -1) 
        
        self.p_down = ((pow(forward_price,-2) * (variance + pow(expectation,2)))- 1 - ((alpha+1) * ((pow(forward_price, -1)*expectation)-1))) / ((1 - alpha) * (pow(alpha,-2) - 1))
        self.p_up = ((pow(forward_price,-1)*expectation)-1-((alpha**(-1)-1)*self.p_down))/(alpha-1)
